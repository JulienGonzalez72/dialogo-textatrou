...
public class ColorComboBox extends JComboBox<String> {
		...
		addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (c == null && canPerform) {
					...
					JOptionPane.showMessageDialog(null, csp, "S�lecteur de couleur", JOptionPane.QUESTION_MESSAGE);
					c = csp.getColor();
				}
				if (c != null) {
					updateBackground(c);
	...
	private class ColorSelectionPanel extends JPanel {
		...
		public ColorSelectionPanel() {
			add(new JLabel("S�lectionnez une couleur :"));
	...
	private static Color getForeground(Color bgColor) {
		// float luminosity = bgColor.getRed() + bgColor.getGreen() * 1.5f +
		// bgColor.getBlue();
